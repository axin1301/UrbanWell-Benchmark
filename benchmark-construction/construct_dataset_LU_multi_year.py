...
def generate_landuse_change_options(lu_from, lu_to, lu_list, similar_map, n_wrong=3):
    ...
    输入:
        lu_from: 起始 landuse
        lu_to: 目标 landuse (正确答案)
        lu_list: 总的 landuse 列表
        similar_map: 相似 landuse 映射 (dict)
        n_wrong: 错误选项数量 (默认 3)
    输出:
        options_str: 格式化的选项字符串 (A. xxx\nB. xxx...)
        correct_idx: 正确答案的选项字母 (A/B/C/D)
    """
    ...
    for sat_image_name, items in grouped_data.items():
        ...
        if len(items) >= used_year_num:
            ...
            **Here are the images for each year:**""",
            ...
            Each year’s imagery is given below:""",
            ...
            for item in selected_items:
                ...
                if len(item['images']) > 1:
                    street_view_images = item['images'][1:number_of_stv+1] # 假设你已经提取了街景图像
                    # 拼接描述
            ...
            new_element = {
                ...
                'prompt': question_template + '\n' + options +'\n' + random.choice(restriction_list), # 假设prompt是相同的
                'references': correct_idx # 合并所有reference
            }
