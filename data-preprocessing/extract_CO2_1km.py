...
for i in range(len(city_csv)-1,len(city_csv)):
    ...
    for year in range(2014,2024):
        ...
        # ==== 参数设置 ====
        tif_file = f"D:\\CitySense原始数据\\ODIAC\\odiac2024_1km_excl_intl_{year_in_dir}07.tif/odiac2024_1km_excl_intl_{year_in_dir}07.tif"
        print(tif_file)
        ...
        if city_name == 'HELSINKI':
            input_csv = "../UrbanAtlas/Helsinki_urbancore_bbox.csv"
        df_bbox = pd.read_csv(input_csv)
        rows_to_process = [0]

        out_dir = f'generated_CO2/{city_name}'
