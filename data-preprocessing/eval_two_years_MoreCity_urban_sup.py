...
def predict_batch(model, img_paths, device, transform):
    ...
    for img_path in img_paths:
        if os.path.getsize(img_path) < 20000:  # 跳过太小的图像
            continue
        try:
    ...
    with torch.no_grad():
        ...
        preds = softmax(preds)[:, 1]  # 取类别 1 概率
...
def process_city_model(city_name, p, model_load_path, model_dict, device):
    ...
    generated_four_points_csv_list = glob.glob(f"../try_GSV/generated_grid_points_urban_sup/{city_name}/*.csv")

    out_Path = "./output_stv_selected_urban_sup/" + city_name
    os.makedirs(out_Path, exist_ok=True)

    # for idx, row in tqdm.tqdm(df_input.iterrows(), total=len(df_input)):
    for generated_four_points_csv in tqdm.tqdm(generated_four_points_csv_list):
        identifier = Path(generated_four_points_csv).stem
        # identifier = str(row['fua_code'])
        # images_path = f"D:\\OneDrive - University of Helsinki\\讨论摘要4\\try_GSV\\downloaded_stv_two_years/{city_name}/{identifier}/street_view_images/"
        # images_path = f"D:\\CitySense原始数据\\try_GSV\\downloaded_stv_selected/{city_name}/{identifier}/street_view_images/"
        images_path = f"D:\\CitySense原始数据\\try_GSV_urban_sup\\downloaded_stv_selected/{city_name}/{identifier}/street_view_images/"
...
if __name__ == "__main__":
    ...
    # city_list = list(pd.read_csv('../UrbanAtlas/City_list_no_helsinki.csv')['city_name'])[1:] ##不包括 HELSINKI
    # city_list.append('HELSINKI')
    city_list = ['HELSINKI']

    # 并行处理每个城市
    with ProcessPoolExecutor(max_workers=2) as executor:  # CPU 核心数可调
        for p in perception:
