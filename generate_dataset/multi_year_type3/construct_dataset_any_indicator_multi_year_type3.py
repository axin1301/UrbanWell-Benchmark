...
for i in tqdm.tqdm(range(len(city_csv))):  #
    ...
    indicators_all = ['landuse_mix','network_density_km_per_km2', 'road_length', 'avg_dist_to_restaurant', 'avg_dist_to_supermarket', 'economic','safety', 'lively', 'wealthy', 'beautiful', 'boring', 'depressing','CO2','NO2','QSI', 'NDVI']
    # 
    ...
    # for indicator in ['landuse_mix','NDVI']:
        ...
        cnt = 0
        data = []
        # output_dir = f'../outputs/multi_year_type3/{city_name}/'
        output_dir = f'../outputs/multi_year_type3/{city_name}/'
        import os
        ...
        for sat_image_name, items in grouped_data.items():
            ...
            if len(items) >= used_year_num:
                ...
                header_templates = [
                    ...
                    f"""Imagine you are studying urban change using geospatial data. The dataset includes {used_year_num} yearly snapshots, from {all_years[0]} through {all_years[-1]}. The images are provided to you as per the instructions below. Each yearâ€™s available imagery is described below:""",

                    f"""You are provided with temporal visual data of a region, covering {used_year_num} years ({all_years[0]} â†’ {all_years[-1]}). The images are provided to you as per the instructions below. For each year, the available images are listed:"""
                ]
...
- "no change" if the change is within Â±{change_threshold}%
...
Provide {used_year_num-1} comma-separated labels (labels include "positive", "negative", and "no change"), nothing else.'''
                ...
                for i in range(len(references)-1):
                    ...
                    delta = (next_val - prev_val) / prev_val  #
